function get_exp_id() {
    ...
    return doc_cookie.slice(doc_cookie.indexOf('uuid=') 
                                    + 'uuid='.length,
                                  doc_cookie.indexOf('uuid=')
                                    + 'uuid='.length + 36);
}
    ...
    var rootRef = new Firebase("https://native-ads.firebaseio.com/"),
      ...
      exp_Ref.once('value', function (snap) {
          ...
          if (current_exp.time_iteration_end == undefined) {
              var iter_duration = current_exp.duration / 5;
          ...
          setInterval( 
                        iterate_experiment(current_exp), 
                        2000
                      );
      });
}
